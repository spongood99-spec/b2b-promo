...
### 알려진 이슈 (2026-08-24 전체 회귀 테스트에서 발견, 미해결)

- **짧은 시간 내 다수 API 연쇄 호출 시 일부 요청의 `req.body`가 빈 값으로 처리됨**: 같은 브라우저 탭에서 `fetch`를 10회 이상 연속 호출하면 정상 payload를 보냈는데도 서버가 "필수 항목이 누락되었습니다"로 응답하는 경우가 관찰됨. 사람이 UI로 폼을 채우고 클릭하는 정상 속도로는 재현되지 않음(등록/비밀번호변경 등 UI 경로 전부 통과). 원인이 애플리케이션 코드인지 Vercel 서버리스 인프라 특성인지 미확정 — 재현조건이 좁아질 때까지 우선순위 낮음으로 보류. 상세: `e2e/E2E-REPORT-FULL-REGRESSION-20260824.md` 2장.

...
### BE-16. (전체 회귀 E2E 후속 조치) 상태전이/유효성/보안 하드닝 4차 (2026-08-24 추가)
- **선행 Task**: BE-15
- **작업 내용**: 정상플로우·경계값·보안·접근성 4개 영역을 나눠 병렬로 수행한 전체 회귀 E2E에서 나온 결함을 조치.
  - 변경요청 처리(`updateChangeRequestStatus`)에 `WHERE apply_status='pending'` 상태 가드 추가 — 이미 처리된 건 재처리 시 409(반복 클릭/뒤로가기로 상태가 뒤집히고 알림이 중복 발송되는 문제 방지)
  - `promotions`/`change_requests`에 `created_at`(둘 다)/`updated_at`(promotions만, DB 트리거로 자동 갱신) 컬럼 추가 — 도메인정의서 5장 Do 규칙(타임스탬프 기록) 준수, `change_requests` 목록 조회에 `ORDER BY created_at` 추가(그전엔 삽입순서에 의존)
  - 프로모션 등록/재제출/수정승인 시 DB로 내려가기 전에 애플리케이션 레벨 검증 추가: 종료일<시작일, 정수 컬럼(moq 등) 소수/int4 초과, 문자열 컬럼(contact_name 등) 길이 초과, 품목명 공백, 품목 50개 초과 — 전부 이전엔 DB 제약 위반으로 500이 나던 경로였다
  - 회원가입 시 회사명(100자)/이메일(255자) 길이 검증 추가(동일한 이유)
  - `errorHandler`가 처리되지 않은 예외의 `err.code`(PostgreSQL 원본 에러코드)를 그대로 노출하던 것을 고정값(`INTERNAL_ERROR`)으로 교체, 단 `22P02`(잘못된 id 형식 등)는 400/`VALIDATION_ERROR`로 재분류
  - access/refresh JWT payload에 `type` 클레임 추가 — 시크릿이 우연히 같아지는 경우에도 refresh token을 access token 자리에서 쓸 수 없도록 방어(전체 회귀 테스트에서 실제로 통과되는 것을 발견)
  - `logout` 컨트롤러의 `clearCookie`가 로그인 때 심은 옵션과 동일한 `sameSite`/`secure`/`httpOnly`를 쓰도록 수정 — 그전에는 운영(크로스사이트, `SameSite=None; Secure`)에서 로그아웃해도 refresh token이 계속 유효했다(Critical)
- **완료 조건**
  - [x] 이미 처리된 변경요청을 다시 처리하면 409가 반환된다
  - [x] 종료일<시작일, moq에 소수/1e10, contact_name 101자, 품목명 빈 문자열, 품목 51개 전부 400을 반환한다(500 아님)
  - [x] 로그인→로그아웃 후 기존 refresh token으로 `/auth/refresh`를 호출하면 401이 반환된다(운영 쿠키 옵션 기준)
...
### FE-16. (전체 회귀 E2E 후속 조치) 접근성/UX 하드닝 4차 (2026-08-24 추가)
- **선행 Task**: FE-14
- **작업 내용**: 접근성·UX 전담 회귀 테스트에서 나온 결함을 조치.
  - `useModalA11y`에 Tab 포커스 트랩 추가(그전엔 Esc닫기/포커스복원만 있고 Tab이 모달 밖으로 빠져나갈 수 있었다)
  - `LoginPage`의 로그인 실패 메시지를 상태코드별로 분기(401만 "이메일 또는 비밀번호가 올바르지 않습니다", 그 외(429 등)는 서버가 보낸 실제 메시지) — 그전엔 원인과 무관하게 항상 같은 문구를 보여줘 rate limit(429)도 "비밀번호가 틀렸다"고 오인시켰다
  - 폼 에러(`.form-error`, 7개 파일)에 `role="alert"` 추가 — 스크린리더가 에러 발생을 자동으로 안내하지 않던 문제
  - `StatusBadge`의 텍스트 색을 고정 짙은색으로, 상태색은 배경 톤+좌측 보더로만 표현(WCAG AA 4.5:1 미달 해결, 예: 승인됨 2.88:1 → 개선)
  - `NotificationBell`의 Esc 닫기 시 포커스가 트리거 버튼으로 복원되도록 수정(그전엔 `<body>`로 빠짐)
  - 캘린더 모바일(375px) 레이아웃을 요일 7열 구조를 유지한 채 셀 크기만 줄이도록 수정(그전엔 1열로 접혀 요일 구분이 불가능했다)
  - 목록에서 품목명이 빈 값인 프로모션의 링크에 "제목 없음" 대체 텍스트 추가(접근 가능한 이름 없는 링크 방지)
  - 이미 로그인된 상태로 `/login`에 접근하면 `/`로 리다이렉트
  - 헤더 내비게이션 링크가 좁은 화면에서 단어 중간에 줄바꿈되던 것을 `white-space: nowrap`으로 수정
- **완료 조건**
  - [x] 비밀번호 변경 모달이 열린 상태에서 Tab을 반복해도 포커스가 모달 밖으로 나가지 않는다
  - [x] rate limit(429)로 로그인이 막히면 "비밀번호가 틀렸다"가 아닌 서버 메시지가 표시된다
  - [x] 알림 드롭다운을 Esc로 닫으면 포커스가 알림 벨 버튼으로 돌아온다
